//
// Created by Ruben Glapa on 3/15/25.
//
...
public:
    ...
    void render(const hittable& world) {
        ...
        for (int j = (image_height/2); j > 0; j--) {
            std::clog << "\rLine remaining: " << (image_height - j) << ' ';
            for (int i = image_width; i > image_width/2; i--) {
...
private:
    ...
    void initialize() {
        ...
        auto const viewport_height = 2 * h * focus_dist;
        ...
        auto const viewport_u = viewport_width * u;
        auto const viewport_v = viewport_height * -v;

        pixel_delta_u = viewport_u / image_width;
        pixel_delta_v = viewport_v / image_height;

        auto const viewport_lower_right = center - (focus_dist * w) - viewport_u/2 - viewport_v/2;
        pixel00_loc = viewport_lower_right + 0.5 * (pixel_delta_u + pixel_delta_v);
    ...
    ray get_ray(int i, int j) const {
        ...
        auto const pixel_sample = pixel00_loc
